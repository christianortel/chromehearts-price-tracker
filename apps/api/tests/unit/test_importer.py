...
def test_stage_csv_imports_rows(db_session: Session) -> None:
    ...
    assert count == 2
    assert db_session.query(LegacyImportRow).filter(LegacyImportRow.source_name == "legacy_csv").count() >= 2
...
def test_publish_staged_row_creates_observation_and_retail_report(db_session: Session) -> None:
    ...
    assert db_session.query(RetailReport).filter(RetailReport.source_observation_id == observation.id).one()
